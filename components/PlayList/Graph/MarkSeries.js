...
export default class MarkSeries extends AbstractSeries {
  ...
  _valueDragEndHandler = (d, event, index) => {
    ...
    if (onValueDragEnd) {
      // console.log('calling onValueDragEnd')
      onValueDragEnd(d, { event, index })
    }
    if (onSeriesDragEnd) {
      // console.log('calling onSeriesDragEnd')
      onSeriesDragEnd({ event })
    }
    if (super._valueDragEndHandler) {
      // console.log('calling super._valueDragEndHandler')
      super._valueDragEndHandler(d, event)
    }
    // console.log('_valueDragEndHandler')
  }
  ...
  toChartCoords = e => {
    ...
    const { xRange, yRange, xDomain, yDomain, innerHeight } = this.props

    const { x: adjX, y: adjY } = this.fromGlobalToChartCoords(e)
